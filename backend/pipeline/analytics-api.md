...
v2.1.2 及更早版本可能将 `combo=false` 的同组累计礼物拆成独立送礼，导致 `1 → 2 → … → 10` 被计为 55 个。升级后新消息使用正确身份，已有错误历史需要离线修复，不能仅靠更新程序消除。

先停止程序。在 Windows 便携版目录打开 PowerShell：
...
Docker 部署请先停止采集和后端写入，使用同一数据库离线运行 `douyin-pipeline --repair-gift-groups`；`DATABASE_PATH` 指向实际数据库。默认只读预览，加 `--apply` 后才写入。应用前在数据库旁创建 `数据库名.before-gift-groups-…/` 完整备份，路径见报告的 `backup`。

仅合并原始载荷与身份证据完整的组，同步历史统计增量、合并展示与房间计数；原始投递数量、单价、时间、事件 ID/序号、用户资料和消息详情保留。全组没有接收者时保留最早的展示身份；存在多个接收者且部分记录缺失接收者时整组跳过。重复执行幂等，应用失败整笔回滚。

查看 `changed_groups`、`gift_quantity_delta`、`skipped_groups` 和 `skip_reasons`。`failed=0` 不代表被跳过的历史也已修好。`stored_count_mismatch` 表示旧数量事实与载荷不一致，应先用下述 `--repair-gift-facts` 检查；其他缺证据、身份冲突或统计不一致的组保持原状。
