  ...
  return <main className="archive-panel" aria-label="所有直播间信息汇总">
    ...
    <div className="archive-tools">
      ...
      <div className="archive-filter-grid">
        <StudioSelect label="汇总直播间筛选" visibleLabel="直播间" value={filters.room} onChange={v => change('room', v)} options={[{ id: '', label: '全部直播间' }, ...rooms.map(room => ({ id: room.live_id, label: roomName(room), description: room.live_id })), ...filters.room && !rooms.some(room => room.live_id === filters.room) ? [{ id: filters.room, label: filters.room, description: '已从监控列表删除' }] : []]} />
